...
- [ ] Hors-ligne : charger l'app une première fois en ligne, puis couper le réseau et recharger la page → l'app doit s'ouvrir (pas d'erreur navigateur) et montrer les dernières notes/clips/recettes connus ; cocher une tâche ou supprimer un élément hors-ligne doit afficher « sera synchronisé au retour du réseau » et un badge « hors-ligne » dans la barre du haut ; remettre le réseau → l'action doit se synchroniser automatiquement (créer un nouvel élément reste volontairement impossible hors-ligne pour l'instant)
- [ ] Navigation simplifiée : la barre du haut ne montre plus que Aujourd'hui / Racine / Graphe / Atelier ; cliquer « Atelier » ouvre un menu avec Presse-papier, Recettes, Rappels, Corbeille ; cliquer un élément y navigue et referme le menu ; cliquer ailleurs ou Échap referme aussi ; le bouton Atelier reste visuellement actif tant qu'on est dans une de ces 4 vues
...
- [ ] La Constellation (onglet Graphe) : les notes sont regroupées visuellement par tag/espace ; les notes isolées (sans lien, sans parent, sans branche) ont un contour en pointillés au lieu d'être pleines ; une note modifiée dans les 3 derniers jours a un halo qui pulse doucement ; des lignes en pointillés jaunes proposent des liens entre notes proches — cliquer dessus crée le lien ; cliquer un nœud puis un second affiche le chemin entre les deux (ou un message "aucun chemin trouvé") ; cliquer deux fois le même nœud l'ouvre ; molette pour zoomer, glisser pour déplacer, bouton "Recentrer" pour tout remettre à zéro ; le filtre temporel (7j/30j/3 mois/tout) doit changer les notes affichées
